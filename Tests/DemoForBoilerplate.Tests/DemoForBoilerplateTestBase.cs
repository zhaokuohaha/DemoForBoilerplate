...
namespace DemoForBoilerplate.Tests
{
    public abstract class DemoForBoilerplateTestBase : AbpIntegratedTestBase
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected DemoForBoilerplateTestBase()
        {
        ...
        protected override void AddModules(ITypeList<AbpModule> modules)
        {
            ...
            modules.Add<DemoForBoilerplateTestModule>();
        }
        ...
        protected void UsingDbContext(Action<DemoForBoilerplateDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<DemoForBoilerplateDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<DemoForBoilerplateDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<DemoForBoilerplateDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<DemoForBoilerplateDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DemoForBoilerplateDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<DemoForBoilerplateDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DemoForBoilerplateDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<DemoForBoilerplateDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<DemoForBoilerplateDbContext, Task<T>> func)
        {
            T result;
